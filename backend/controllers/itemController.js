...
      imageUrls = req.files.map((file) => "/public/uploads/items/" + file.filename);
    }

    // Manually parse the location field from the JSON string
    let parsedLocation = {};
    try {
...
      // If location parsing fails, validation will likely catch it later
    }
...
      owner: TEMP_OWNER_ID,
      images: imageUrls,
      // Overwrite the location string with the parsed object
